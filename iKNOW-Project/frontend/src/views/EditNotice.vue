          ...
          <Col span="15">
            <div class="layout-content-main">
              <Form ref="infoForm" :model="infoForm" :rules="ruleinfoForm" :label-width="80">
                <Row>
                  <Col span="5" class="font-style">修改公告</Col>
                </Row>
...
export default {
  ...
  created () {
    ...
    .catch((response) => {
      this.$Message.error('抱歉，获取公告信息失败')
    })
  },
  methods: {
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.$http.post(this.GLOBAL.baseUrl + 'api/competition/notice_detail/' + this.$route.params.id, {title: this.infoForm.title, content: this.infoForm.content, status: 1})
          .then((response) => {
    ...
    handleSave (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.$http.post(this.GLOBAL.baseUrl + 'api/competition/notice_detail/' + this.$route.params.id, {title: this.infoForm.title, content: this.infoForm.content, status: 0})
          .then((response) => {
